jQuery(document).ready(function() {
    ...
    jQuery('body').on('click', '#submit_search', function(e) { 
        ...
        var $form            = jQuery('#searchbox_form'),  
        ...
        var url = submit_url(brand, page, origin_iata, destination_iata, origin_full, destination_full, period);
        window.open(url);
...
function submit_url(brand, page_ext, origin, destination, origin_full, destination_full, flight_period) {
    
    if(origin === "") origin = origin_full;
    if(destination === "") destination = destination_full;
    
    var url_base, url_parameters, url_tracking;
    ...
    switch(brand) {
        case "travellink":
            ...
            url_parameters = '?type=Flow' + 
                ...
                '&ClassOfService=E' + 
                '&D_Day=' + get_date_format(flight_period, 'D', '') + // DD
                '&D_Month=' + get_date_format(flight_period, 'Y', '') + get_date_format(flight_period, 'M', '') + // YYYYMM
                '&D_Time=TANY' +
                '&D_City=' + origin +                 
                '&R_Day=' + get_date_format(flight_period, 'D', 'return') + // get_date_format(flight_period, 'D')
                '&R_Month=' + get_date_format(flight_period, 'Y', 'return') + get_date_format(flight_period, 'M', 'return') + // YYYYMM get_date_format(flight_period, 'Y', 'return') + get_date_format(flight_period, 'M', 'return')
                '&R_Time=TANY' +
                '&A_City=' + destination + 
                '&NR_OF_ADT=1' +
                '&NR_OF_CHD=0' + 
            ...
            break;
        default:
            url_base = 'http://www.' + brand + '.' + page_ext + '/engine/waitPage/waitingPage.jsp?action=engine/searchOffers/search',        
            url_parameters = '&departureDate=' +
